...
public class TopographicMap
{
    ...
    private int DescendFromPoint(int row, int col, HashSet<(int row, int col)>? trailheads = null)
    {
        trailheads ??= [];
        ...
        {
            // found a trailhead
            return trailheads.Add((row, col)) ? 1 : 0;
        }
        ...
        foreach (var (nRow, nCol) in neighbors)
        {
            ...
            if (nValue == value - 1)
            {
                paths += DescendFromPoint(nRow, nCol, trailheads); // keep descending
            }
        }
